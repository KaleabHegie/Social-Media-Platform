...
export default {
  ...
  methods: {
    ...
    validateForm() {
      ...
      if (!emailRegex.test(this.form.username) && this.form.username.length < 3) {
        this.errors.username = 'እባክዎ የሚሰራ ኢሜይል ወይም የተጠቃሚ ስም ያስገቡ';
      }

      // Password validation
      const passwordRegex = /^(?=.*[A-Za-z])(?=.*\d)(?=.*[@$!%*#?&])[A-Za-z\d@$!%*#?&]{8,}$/;
      if (!passwordRegex.test(this.form.password)) {
        this.errors.password = 'Password must be at least 8 characters long and include letters, numbers, and special characters.';
      }
