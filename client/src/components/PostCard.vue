<template>
  ...
  <article class="bg-white dark:bg-gray-800 rounded-xl shadow-lg mb-6 overflow-hidden">
    ...
    <div class="relative fade-mask">
        ...
        <div class="flex items-center">
          <router-link :to="`/user/${props.post.username}`" class="flex items-center group">
            <div class="relative w-10 h-10 rounded-full overflow-hidden border-2 border-white">
              <img :src="props.post.userAvatar || '/default-avatar.png'" :alt="props.post.username"
      ...
      <div class="relative">
        <div
          v-if="props.post.medias && props.post.medias.length"
          class="flex transition-transform duration-300 ease-in-out"
          :style="{ transform: `translateX(-${activeSlide * 100}%)` }"
        >
        
          <div v-for="(media, index) in props.post.medias" :key="index" class="w-full flex-shrink-0">
        ...
        <!-- Carousel Navigation -->
        <div
          v-if="props.post.media && props.post.media.length > 1"
    ...
    <div class="p-4">
      ...
      <div class="flex items-center justify-between mb-4">
        <div class="flex items-center space-x-4">
          <button @click="toggleLike" class="flex items-center space-x-2 group" :aria-label="isLiked ? 'Unlike post' : 'Like post'">
            ...
            <span class="text-sm text-gray-600 dark:text-gray-400">{{ props.post.likes_count || 0 }}</span>
          </button>

          <router-link :to="`/viewPost`" class="flex items-center space-x-2 group" aria-label="View comments">
